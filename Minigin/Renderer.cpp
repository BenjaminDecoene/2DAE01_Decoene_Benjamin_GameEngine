...
void dae::Renderer::Render(float interpolation) const
{
...
void dae::Renderer::RenderTexture(const Texture2D& texture, const float x, const float y, const SDL_Rect& src, const float width, const float height, const float angle) const
{
	SDL_Rect dst;
	dst.x = static_cast<int>(x);
	dst.y = static_cast<int>(y);
